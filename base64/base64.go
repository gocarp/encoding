...
// Package base64 provides useful API for BASE64 encoding/decoding algorithm.
package base64
...
func EncodeFile(path string) ([]byte, error) {
	...
	if err != nil {
		err = errors.Wrapf(err, `os.ReadFile failed for filename "%s"`, path)
		return nil, err
	}
...
func Decode(data []byte) ([]byte, error) {
	...
	if err != nil {
		err = errors.Wrap(err, `base64.StdEncoding.Decode failed`)
	}
	return src[:n], err
}
