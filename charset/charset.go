...
func Convert(dstCharset string, srcCharset string, src string) (dst string, err error) {
	...
	if srcCharset != "UTF-8" {
		if e := getEncoding(srcCharset); e != nil {
			...
			return dst, errors.NewCodef(codes.CodeInvalidParameter, `unsupported srcCharset "%s"`, srcCharset)
		}
	}
	...
	if dstCharset != "UTF-8" {
		if e := getEncoding(dstCharset); e != nil {
			...
			return dst, errors.NewCodef(codes.CodeInvalidParameter, `unsupported dstCharset "%s"`, dstCharset)
		}
	} else {
		dst = src
	}
